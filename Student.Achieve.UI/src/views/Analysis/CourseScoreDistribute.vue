<template>
    <section>
        ...
        <dv-scroll-ranking-board :config="config" style="width: 500px; height: 300px; color: black" v-show="is_search" />

        <!--列表-->
        <el-table :data="CourseScoreDistribute"
    ...
    getCourseScoreDistribute() {
      ...
      this.listLoading = true;
        ...
        this.total = res.data.response.dataCount;
          ...
          console.log(this.CourseScoreDistribute);
          let dataArray = [];
          for (let i = 0; i <= this.CourseScoreDistribute.length - 1; i++) {
              dataArray.push({
                  name: this.CourseScoreDistribute[i].Clazz+"班",
                  value: this.CourseScoreDistribute[i].C_Pass_Rate
              });
          }
          this.config.data = dataArray;
